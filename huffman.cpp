...
unsigned char Node::find_path(std::map<unsigned char, std::vector<unsigned char>>& paths, Character_and_codes& codes, std::vector<unsigned char>& path) const {
    if (is_letter_node) {
...
std::pair<unsigned char, bool> find_in_tree(Node* current_node, std::vector<char>& path, std::uint32_t current_byte) {
    if (current_byte >= static_cast<std::uint32_t>(path.size()))
        return {0, false};
    if (current_node -> is_character())
        return {current_node -> get_character(), true};
    if (path[current_byte] == 0) {
        return find_in_tree(current_node -> get_left_child(), path, current_byte + 1);
    }
    else {
        return find_in_tree(current_node -> get_right_child(), path, current_byte + 1);
    }
}
//insert a new Node in the tree by the path to it
void insert_in_tree(unsigned char character, Node* current_node, std::vector<unsigned char>& path, std::uint32_t current_byte) {
    if (current_byte == static_cast<std::uint32_t>(path.size()))
        return;
    Node* new_direction;
    if (path[current_byte] == 1) {
        if (current_node -> get_right_child()) {
            new_direction = current_node -> get_right_child();
        }
        else {
            current_node -> set_right_child(new Node());
            new_direction = current_node -> get_right_child();
        }
    } else {
        if (current_node -> get_left_child()) {
            new_direction = current_node -> get_left_child();
        }
        else {
            current_node -> set_left_child(new Node());
            new_direction = current_node -> get_left_child();
        }
    }
    if (current_byte == static_cast<std::uint32_t>(path.size()) - 1) {
        new_direction -> set_character(character);
        new_direction -> set_is_character();
        return;
    }
    insert_in_tree(character, new_direction, path, current_byte + 1);
}
...
std::pair<std::map<unsigned char, std::uint32_t>, std::uint32_t> get_map_of_characters(const std::string& path, std::uint32_t& total_input) {
    ...
    if (fin.is_open()) {
        while(!(fin.eof())) {
            ...
            current_char = buff[0];
            ++frequency;
            if (map_of_frequencies.find(current_char) != map_of_frequencies.end()) {
                ++map_of_frequencies[current_char];
            }
            else {
                map_of_frequencies.insert({current_char, 1});
            }
        }
    }
    return {map_of_frequencies, frequency};
}
unsigned char get_byte(const Bite_array& arr) {
...
void write_to_file_binary(const std::string& f_in_path, const std::string& f_out_path, Character_and_codes& codes, std::uint32_t number_of_ch, std::uint32_t& total_aux, std::uint32_t& total_coded) {
...
Bite_array get_array_from_int(unsigned char current_char) {
    Bite_array resulting_array;
    resulting_array.fill(false);
    if (current_char == 0) {
        return resulting_array;
    }
    std::uint32_t j = 1;
    while (current_char > 0) {
        resulting_array[8 - j] = ((current_char % 2) == 1);
        current_char = current_char / 2;
        ++j;
    }
    return resulting_array;
}
void print_sorted_codes(const Character_and_codes& codes) {
...
Node* get_tree_of_codes(std::ifstream& fin, std::uint32_t& total_number, std::uint32_t& number_of_ch, std::uint32_t& total_aux) {
    ...
    if (fin.is_open()) {
        ...
        if (!fin.eof()) {
            fin >> total_number;
            fin.read(reinterpret_cast<char*>(buff), 1);
            for (std::uint32_t i = 0; i < static_cast<std::uint32_t>(total_number); ++i) {
...
Character_and_codes get_sorted_codes_from_tree(Node* root_of_tree, std::uint32_t& total_input, std::uint32_t& number_of_ch) {
    Character_and_codes sorted_codes;
...
std::uint32_t huffman_decode(const std::string& f_in_path, const std::string& f_out_path, bool flag_v) {
    ...
    std::ifstream fin(f_in_path, std::ios::binary);
    std::ofstream fout(f_out_path, std::ios::binary);
    std::uint32_t total_number = 0;
    std::uint32_t number_of_ch = 0;
    Node* root_of_tree = get_tree_of_codes(fin, total_number, number_of_ch, total_aux);
    print_to_file_decoded_characters(fin, fout, root_of_tree, number_of_ch, total_input);
    total_coded = static_cast<std::uint32_t>(fout.tellp());
    total_input -= total_aux;
    Character_and_codes sorted_codes = get_sorted_codes_from_tree(root_of_tree, total_aux, total_number);
    delete root_of_tree;
...
Node* get_tree_of_characters(const std::string& fin_path, std::uint32_t& number_of_ch, bool& flag, std::uint32_t& total_input) {
    ...
    std::priority_queue<Node*, std::vector<Node*>, less_than_by_frequency> current_priority_queue;
...
Character_and_codes get_sorted_codes(const std::string& fin_path, std::uint32_t& number_of_ch, bool& flag, std::uint32_t& total_input) {
    Character_and_codes sorted_codes;
...
void huffman(const std::string& fin_path, const std::string& fout_path, bool flag_v) {
    ...
    std::uint32_t total_aux = 0;
    std::uint32_t total_coded = 0;
    Character_and_codes sorted_codes  = get_sorted_codes(fin_path, number_of_ch, flag, total_input);
